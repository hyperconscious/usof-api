...
export class MailService {
  public async sendVerificationEmail(email: string, token: string) {
    const verificationLink = `http://${config.host}:${config.port}}/verify-email?token=${token}`;
  ...
  public async sendPasswordResetEmail(email: string, token: string) {
    const resetLink = `http://${config.host}:${config.port}}/reset-password?token=${token}`;
    ...
    const mailOptions = {
      ...
      html: `<p>You can reset your password by folowing <a href="${resetLink}">this link</a>.</p>`,
    };
