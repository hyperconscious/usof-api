...
export class Tables1729776100085 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`login\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`full_name\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`verified\` tinyint NOT NULL DEFAULT 0, \`avatar\` varchar(255) NOT NULL DEFAULT '', \`publisher_rating\` int NOT NULL DEFAULT '0', \`commentator_rating\` int NOT NULL DEFAULT '0', \`role\` enum ('admin', 'user') NOT NULL DEFAULT 'user', \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_a62473490b3e4578fd683235c5\` (\`login\`), UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), UNIQUE INDEX \`IDX_bec1a7d56ad6e762a1e2ce527b\` (\`login\`, \`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`category\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`description\` text NULL, UNIQUE INDEX \`IDX_9f16dbbf263b0af0f03637fa7b\` (\`title\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`comment\` (\`id\` int NOT NULL AUTO_INCREMENT, \`publish_date\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`content\` text NOT NULL, \`likes_count\` int NOT NULL DEFAULT '0', \`dislikes_count\` int NOT NULL DEFAULT '0', \`user_id\` int NULL, \`post_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`post\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`publishDate\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`status\` enum ('active', 'inactive') NOT NULL DEFAULT 'inactive', \`content\` text NOT NULL, \`likes_count\` int NOT NULL DEFAULT '0', \`dislikes_count\` int NOT NULL DEFAULT '0', \`comments_count\` int NOT NULL DEFAULT '0', \`user_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`like\` (\`id\` int NOT NULL AUTO_INCREMENT, \`publish_date\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`entity_type\` varchar(255) NOT NULL DEFAULT 'post', \`type\` varchar(255) NOT NULL DEFAULT 'like', \`user_id\` int NULL, \`post_id\` int NULL, \`comment_id\` int NULL, UNIQUE INDEX \`IDX_de18085ee8cf871497f08209d2\` (\`user_id\`, \`post_id\`, \`comment_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`IDX_de18085ee8cf871497f08209d2\` ON \`like\``);
